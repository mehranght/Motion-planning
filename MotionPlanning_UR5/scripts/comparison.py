...
def main(config, weights):
    ...
    for j in range(n_traj):
        ...
        while True:
            ...
            set_joint_positions(robot, joints, initial_configuration_rads)
        ...
        current_configuration = torch.FloatTensor(initial_configuration_rads)
        tic = time.time()
        path_model = MPnetPath(current_configuration,goal_configuration,model,collision_fn,0.05,plan_oracle,robot,obstacles,joints,CUSTOM_LIMITS)
        toc = time.time()
        
        tic1 = time.time()
        path_oracle = plan_path1(
                robot=robot,
                joints=joints,
                collision_fn=collision_fn,
                custom_limits=CUSTOM_LIMITS,
                obstacles=obstacles,
                initial_configuration_rads=initial_configuration_rads,
